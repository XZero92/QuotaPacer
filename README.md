...
- `small` 152×56 캡슐 안의 원형 게이지, `middle` 280×72 바 게이지, `large` 320px 폭 균등 페이스 카드
...
- 제한 기간 경과율과 소진율을 비교한 무저장 균등 페이스 표시
- 60초 주기 갱신 및 `account/rateLimits/updated` 이벤트 후 500ms 디바운스 갱신
- 오류나 연결 끊김 시 마지막 성공 값을 흐리게 유지하고 재연결
- 크기·창 위치 저장, 모니터 작업 영역 보정, 트레이 표시/숨기기·새로고침·CLI 경로 선택·종료

현재 범위에는 자동 시작, 운영체제 알림, 캐릭터 애니메이션, 사용량 기록·차트, 앱 내부 로그인, CLI 번들링이 포함되지 않습니다.
...
앱은 Codex 인증 토큰, 계정 이메일, 사용량 스냅샷을 디스크에 저장하지 않습니다. 디스크에는 사용자가 지정한 CLI 경로와 오버레이 위치만 저장합니다. API Key나 지원하지 않는 인증 방식은 별도 상태로 안내하며 앱 내부 로그인은 제공하지 않습니다.
