...
- 세 가지 정보 밀도의 항상 위 오버레이: Small·Middle은 300분 제한 창을 우선 표시하되 Luna Reserve 우선 표시 시 Reserve로 전환, Large는 일반 창과 Luna Reserve의 현재 사용 가능 상태를 구분해 표시
- 24시간 미만 창의 관측 신뢰도·소진 위험과 24시간 이상 창의 사용 계획·예상 소진 비교
- 확정된 위험에 대한 인라인 경고와 선택적 OS 알림
- 주기적 갱신, 변경 이벤트 반영, 연결 끊김 시 마지막 값 유지와 자동 재연결
- 드래그 이동, 투명도·언어·페이스 설정과 창 위치 저장
- 선택적으로 시스템 로그인 시 QuotaPacer와 오버레이 자동 실행
...
- 자동 시작은 운영체제의 로그인 항목을 사용하며 설치 패키지에서 실제 재로그인 동작을 확인해야 합니다.
